...
##### Regras de transmissão
    ...
    # A transmissão permite que funções universais lidem de maneira significativa com entradas que não têm exatamente a mesma forma.

    # A primeira regra de transmissão é que se todos os arrays de entrada não tiverem o mesmo número de dimensões, a “1” será repetidamente anexado às formas dos arrays menores até que todos os arrays tenham o mesmo número de dimensões.

    # A segunda regra de transmissão garante que arrays com tamanho 1 ao longo de uma dimensão específica ajam como se tivessem o tamanho do array com a maior forma ao longo dessa dimensão. O valor do elemento da matriz é considerado o mesmo ao longo dessa dimensão para a matriz “broadcast”.

    # Após a aplicação das regras de transmissão, os tamanhos de todos os arrays devem corresponder. Mais detalhes podem ser encontrados em Transmissão.





 ########## Indexação avançada e truques de índice ##########   
    
    # O NumPy oferece mais recursos de indexação do que as sequências regulares do Python. Além da indexação por inteiros e fatias, como vimos anteriormente, os arrays podem ser indexados por arrays de inteiros e arrays de booleanos.


##### Indexação com matrizes de índices 

a = np.arange(12)**2  # os primeiros 12 números quadrados
i = np.array([1, 1, 3, 8, 5])  # uma matriz de índices
a[i]  # os elementos de `a` nas posições `i`

j = np.array([[3, 4], [9, 7]])  # uma matriz bidimensional de índices
a[j]  # a mesma forma que `j` 

    # Quando a matriz indexada a é multidimensional, uma única matriz de índices se refere à primeira dimensão de a. O exemplo a seguir mostra esse comportamento convertendo uma imagem de rótulos em uma imagem colorida usando uma paleta. 
...
palette = np.array([[0, 0, 0],         # Preto
                    ...
                    [0, 0, 255],       # azul
                    [255, 255, 255]])  # Branco
image = np.array([[0, 1, 2, 0],  # cada valor corresponde a uma cor na paleta
                  [0, 3, 4, 0]])
palette[image]  # a (2, 4, 3) imagem colorida 

    # Também podemos fornecer índices para mais de uma dimensão. As matrizes de índices para cada dimensão devem ter a mesma forma. 


a = np.arange(12).reshape(3, 4)
a

i = np.array([[0, 1],  # índices para o primeiro dim de `a`
              [1, 2]])
j = np.array([[2, 1],  # índices para o segundo dim
              [3, 3]])

a[i, j]  # i e j devem ter a mesma forma 
...
a[:, j]
    ...
    # Em Python, arr[i, j] é exatamente o mesmo que arr[(i, j)]—então podemos colocar i e j em uma tupla e então fazer a indexação com isso. 
...
a[l]
    ...
    # No entanto, não podemos fazer isso colocando i e j em um array, porque esse array será interpretado como indexando a primeira dimensão de a. 
...
a[tuple(s)]
    ...
    # Outro uso comum da indexação com arrays é a busca do valor máximo de séries dependentes do tempo: 



time = np.linspace(20, 145, 5)  # escala de tempo
data = np.sin(np.arange(20)).reshape(5, 4)  # 4 séries dependentes do tempo
time


data

# índice dos máximos para cada série
ind = data.argmax(axis=0)
ind

# vezes correspondentes ao máximo 
time_max = time[ind]
...
np.all(data_max == data.max(axis=0))
    ...
    # Você também pode usar a indexação com arrays como um destino para atribuir a: 
...
a
    ...
    # No entanto, quando a lista de índices contém repetições, a atribuição é feita várias vezes, deixando para trás o último valor: 

a = np.arange(5)
a[[0, 0, 2]] = [1, 2, 3]
a

    # Isso é bastante razoável, mas cuidado se você quiser usar a construção += do Python, pois ela pode não fazer o que você espera: 

a = np.arange(5)
a[[0, 0, 2]] += 1
a

    # Embora 0 ocorra duas vezes na lista de índices, o elemento 0 é incrementado apenas uma vez. Isso ocorre porque o Python exige que a += 1 seja equivalente a a = a + 1. 



##### Indexação com Arrays Booleanos

    # Quando indexamos arrays com arrays de índices (inteiros), estamos fornecendo a lista de índices a serem escolhidos. Com índices booleanos a abordagem é diferente; escolhemos explicitamente quais itens do array queremos e quais não queremos.

    # A maneira mais natural que se pode pensar para a indexação booleana é usar arrays booleanos que tenham a mesma forma que o array original: 

a = np.arange(12).reshape(3, 4)
b = a > 4
b  # `b` é um booleano com a forma de `a`


a[b]  # 1d array com os elementos selecionados 

    # Esta propriedade pode ser muito útil em atribuições:

a[b] = 0  # Todos os elementos de `a` maiores que 4 se tornam 0 
a

    # Você pode ver o exemplo a seguir para ver como usar a indexação booleana para gerar uma imagem do conjunto Mandelbrot: 

import numpy as np
import matplotlib.pyplot as plt
def mandelbrot(h, w, maxit=20, r=2):
    """Retorna uma imagem do fractal Mandelbrot de tamanho (h,w).""" 
    ...
    for i in range(maxit):
        z = z**2 + C
        diverge = abs(z) > r                    # quem está divergindo
        div_now = diverge & (divtime == maxit)  # quem está divergindo agora
        divtime[div_now] = i                    # note quando
        z[diverge] = r                          # evite divergir muito 
...
plt.imshow(mandelbrot(400, 400))
    ...
    # A segunda forma de indexação com booleanos é mais semelhante à indexação de inteiros; para cada dimensão do array damos um array booleano 1D selecionando as fatias que queremos: 

a = np.arange(12).reshape(3, 4)
b1 = np.array([False, True, True])         # primeira seleção de escurecimento
b2 = np.array([True, False, True, False])  # segunda seleção de escurecimento

a[b1, :]                                   # selecionando linhas

a[b1]                                      # mesma coisa

a[:, b2]                                   # selecionando colunas

a[b1, b2]                                  # uma coisa estranha de se fazer 


    # Observe que o comprimento da matriz booleana 1D deve coincidir com o comprimento da dimensão (ou eixo) que você deseja fatiar. No exemplo anterior, b1 tem comprimento 3 (o número de linhas em a), e b2 (de comprimento 4) é adequado para indexar o 2º eixo (colunas) de a.



##### A função ix_() 

    # A função ix_ pode ser usada para combinar diferentes vetores para obter o resultado para cada n-uplet. Por exemplo, se você quiser calcular todos os a+b*c para todos os trigêmeos retirados de cada um dos vetores a, b e c: 
...
a[3] + b[2] * c[4]
    ...
    # Você também pode implementar a redução da seguinte forma: 
...
ufunc_reduce(np.add, a, b, c)
    ...
    # A vantagem desta versão do reduce em relação ao ufunc.reduce normal é que ela faz uso das regras de broadcast para evitar a criação de um array de argumentos com o tamanho da saída vezes o número de vetores. 
...
########## Truques e dicas  ##########
    ...
    # Aqui damos uma lista de dicas curtas e úteis.



##### Remodelação “Automática”

    # Para alterar as dimensões de um array, você pode omitir um dos tamanhos que serão deduzidos automaticamente: 

a = np.arange(30)
b = a.reshape((2, -1, 3))  # -1 significa "o que for necessário" 
b.shape
...
##### Empilhamento de vetores

    # Como construímos uma matriz 2D a partir de uma lista de vetores de linha de tamanho igual? No MATLAB isso é bem fácil: se x e y são dois vetores de mesmo comprimento, você só precisa fazer m=[x;y]. No NumPy isso funciona através das funções column_stack, dstack, hstack e vstack, dependendo da dimensão em que o empilhamento deve ser feito. Por exemplo: 
...
xy
    ...
    # A lógica por trás dessas funções em mais de duas dimensões pode ser estranha. 

##### Histogramas 

    # A função de histograma NumPy aplicada a um array retorna um par de vetores: o histograma do array e um vetor das arestas do bin. Cuidado: o matplotlib também tem uma função para construir histogramas (chamado hist, como no Matlab) que difere do NumPy. A principal diferença é que pylab.hist plota o histograma automaticamente, enquanto numpy.histogram apenas gera os dados. 

import numpy as np
rg = np.random.default_rng(1)
import matplotlib.pyplot as plt
# Construir um vetor de 10.000 desvios normais com variância 0,5^2 e média 2
mu, sigma = 2, 0.5
v = rg.normal(mu, sigma, 10000)
# Plota um histograma normalizado com 50 bins
plt.hist(v, bins=50, density=True)       # versão matplotlib (plot)

# Calcula o histograma com numpy e, em seguida, plota-o
(n, bins) = np.histogram(v, bins=50, density=True)  # Versão NumPy (sem plot) 
plt.plot(.5 * (bins[1:] + bins[:-1]), n) 

    # Com Matplotlib >=3.4 você também pode usar plt.stairs(n, bins). 
...
########## Leitura adicional ##########
    ...
    # O tutorial do Python (https://docs.python.org/tutorial/)

    # Referência NumPy (https://numpy.org/devdocs/reference/index.html#reference)

    # SciPy Tutorial (https://docs.scipy.org/doc/scipy/reference/tutorial/index.html)

    # Notas de aula do SciPy (https://scipy-lectures.org/)

    # Um dicionário matlab, R, IDL, NumPy/SciPy (http://mathesaurus.sf.net/)
